# V2 API: `client/player/me/get-task-group-status`

**Endpoint:** `POST /v2/client/player/me/get-task-group-status`
...
```json
{
  "status": "success",
  "code": 200,
  "message": "Get My Task Group Status List",
  "errors": [],
  "data": {
    "taskGroupStatuses": [
      {
...
|-------|------|-------------|
| `data.taskGroupStatuses` | `array` | Array of task group status objects |
...
| `tasks[].status` | `string` | Task status (e.g., `"pending"`, `"completed"`, `"reward_claimed"`) |
...
- **DTO**: `src/task/dtos/get-my-task-group-status.v2.dto.ts`
- **Controller**: `src/task/task.controller.ts` (`getMyTaskGroupStatusV2`)
- **Service**: `src/task/task.service.ts` (`getTaskGroupStatusV2`)
- **Swagger**: `src/task/task.swagger.ts` (`GetMyTaskGroupStatusV2SuccessResponse`)
