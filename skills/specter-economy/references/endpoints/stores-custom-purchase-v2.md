# V2 API: `client/stores/custom-purchase`

**Endpoint:** `POST /v2/client/stores/custom-purchase`
...
|-------|------|----------|-------------|
| `items` | `Item[]` | No* | Array of items to purchase with custom prices |
| `bundles` | `Bundle[]` | No* | Array of bundles to purchase with custom prices |
...
| `price` | `number` | **Yes** | Custom price to charge (positive number) |
    ...
    "bundlesFailed": [
      {
        ...
        "collectionId": "default",
        "stackId": "default",
        "message": "Bundle is not added to collection",
...
### Example 1: Purchase Item with Discounted Price
**Request:**
```json
{
  "items": [
    ...
    "items": [
      {
        ...
        "description": "A gift for new players",
        "iconUrl": "https://cdn.example.com/gift.png",
        "rarity": null,
        "quantity": 1,
        "collectionId": "default",
        "stackId": "default",
...
### Example 3: Mixed Purchase with Custom Prices
**Request:**
```json
{
  "items": [
...
- Allows specifying a **custom price** for each item/bundle, overriding the default store price
- Use this for:
  ...
  - Dynamic pricing
  - Free items (price = 0)
- The `price` field is **required** for each item/bundle
- For standard pricing, use `stores/default-purchase` endpoint instead
- Track discount information in `customParams` for analytics purposes
...
- **DTO**: `src/inventory/dtos/custom-purchase.v2.dto.ts`
